 ...
 *******************************************************************************/
void Stitcher::DrawInterestPoints(CIntPt *interestPts, int numInterestPts, QImage &imageDisplay)
{
    /*int i;
    int r, c, rd, cd;
 ...
 *******************************************************************************/
void Stitcher::ComputeDescriptors(QImage image, CIntPt *interestPts, int numInterestPts)
{
    if(!numInterestPts)
	{
		return;
 ...
 *******************************************************************************/
void Stitcher::DrawMatches(CMatches *matches, int numMatches, QImage &image1Display, QImage &image2Display)
{
   /* int i;
 ...
 *******************************************************************************/
bool Stitcher::ComputeHomography(CMatches *matches, int numMatches, double h[3][3], bool isForward)
{
    int error;
...
void testDraw(double imageToDraw[], int imageToDrawWidth, int imageToDrawHeight, QImage &imageDisplay)
{
	...
	for(int r = 0; r < imageDisplay->Height(); r++)
	{
		for(int c = 0; c < imageDisplay->Width(); c++)
		{
			imageDisplay->SetPixel(c, r, Pixel((int) imageToDraw[r * imageToDrawWidth + c] * factor, (int) imageToDraw[r * imageToDrawWidth + c] * factor, (int) imageToDraw[r * imageToDrawWidth + c] * factor));
		}
	}
}
 ...
 *******************************************************************************/
void Stitcher::SeparableGaussianBlurImage(double *image, int imageWidth, int imageHeight, double sigma)
{
 ...
 *******************************************************************************/
void Stitcher::HarrisCornerDetector(QImage image, double sigma, double thres, CIntPt **interestPts, int &numInterestPts)
{
    int r, c;
    ...
    for(r = 0; r < imageHeight; r++) {
        for(c = 0; c < imageWidth; c++) {
			buffer[r * imageWidth + c] = image->Pixel(c, r).g;
        }
	}
 ...
 *******************************************************************************/
void Stitcher::MatchInterestPoints(QImage image1, CIntPt *interestPts1, int numInterestPts1,
 ...
 *******************************************************************************/
int Stitcher::ComputeInlierCount(double h[3][3], CMatches *matches, int numMatches, double inlierThreshold)
{
 ...
 *******************************************************************************/
void Stitcher::RANSAC(CMatches *matches, int numMatches, int numIterations, double inlierThreshold,
                        double hom[3][3], double homInv[3][3])
{
 ...
 *******************************************************************************/
bool Stitcher::BilinearInterpolation(QImage image, double newX, double newY, double rgb[3])
{
	...
	Pixel p1 = image->Pixel(x, y);
	Pixel p2 = image->Pixel(x + 1, y);
	Pixel p3 = image->Pixel(x, y + 1);
	Pixel p4 = image->Pixel(x + 1, y + 1);
    
	// Use interpolation formula to calculate the color channels for f(x+a, y+b)
	rgb[0] = (1.0 - a) * (1.0 - b) * p1.r
    + a * (1 - b) * p2.r
    + (1 - a) * b * p3.r
    + a * b * p4.r;
	rgb[1] = (1.0 - a) * (1.0 - b) * p1.g
    + a * (1 - b) * p2.g
    + (1 - a) * b * p3.g
    + a * b * p4.g;
	rgb[2] = (1.0 - a) * (1.0 - b) * p1.b
    + a * (1 - b) * p2.b
    + (1 - a) * b * p3.b
    + a * b * p4.b;
 ...
 *******************************************************************************/
void Stitcher::Stitch(QImage image1, QImage image2, double hom[3][3], double homInv[3][3], QImage &stitchedImage)
{
	...
	sWidth = (size_t) ceil(max((double) image1->Width(), max(image2CornerB_x_proj, image2CornerD_x_proj))
                        - min(0.0, min(image2CornerA_x_proj, image2CornerC_x_proj)));
	sHeight = (size_t) ceil(max((double) image1->Height(), max(image2CornerC_y_proj, image2CornerD_y_proj))
                         - min(0.0, min(image2CornerA_y_proj, image2CornerB_y_proj)));
    
    stitchedImage = new Image(sWidth, sHeight);
    
    assert(stitchedImage->Width() == sWidth);
    assert(stitchedImage->Height() == sHeight);
	...
	for(int row = 0; row < stitchedImage->Height(); row++)
	{
		for(int col = 0; col < stitchedImage->Width(); col++)
		{
               ...
               (stitchedImage_y_proj < image2->Height()))
			{
                ...
                Pixel pixel = stitchedImage->Pixel(col, row);
				if(pixel != Pixel(0, 0, 0)) {
					// Weight each image equally
					color[0] = 0.5 * color[0] + 0.5 * pixel.r;
					color[1] = 0.5 * color[1] + 0.5 * pixel.g;
					color[2] = 0.5 * color[2] + 0.5 * pixel.b;
				}
				else
				{
					// Just use the image2 colors as-is
				}
				stitchedImage->SetPixel(col, row, Pixel((int) color[0], (int) color[1], (int) color[2]));
			}
		}
	}
}
